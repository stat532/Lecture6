...
1. Model Specification: *a sampling model $p(y_1, \dots, y_n | \theta)$ is specified. Where $\theta$ could be a high dimensional parameter set.*
	\vfill
2. Prior Specification: *a probability distribution $p(\theta)$ is specified.* 
...
\vfill
3. Posterior Summary: A description of the posterior distribution $p(\theta|y_1, \dots, y_n)$, typically using intervals, posterior means, and predictive probabilities.
\vfill
...
- *are not models,*
\vfill
- *they do not create more information than what is in the data $y_1, \dots, y_n$ and the prior $p(\theta)$,*
\vfill
- *they are a way of "looking at" the posterior $p(\theta|y_1, \dots, y_n)$*
\vfill
...
\vfill
\newpage
To keep the distinction between *estimation* and *approximation* clear, commonly *estimation* is used do describe how we use $p(\phi|y_1, \dots, y_n)$ to make inferences about $\phi$ and *approximation* is used to describe the use of Monte Carlo (including MCMC) procedures to approximate integrals.
\vfill
...
\vfill
1. *The particle move out of $A_2$ and into higher probability regions, and*
\vfill
...
where $\bar{\phi} = \sum_{j=1}^J \phi^{(j)} / J$. *This is due to the central limit theorem.* 
...
\vfill
0. *$\theta^{(0)}$ - initialization*
\vfill
1. *Run multiple chains from different starting points*
\vfill
2. *Assess the similarity of different chains, visually and/or with a test statistic*
...
\vfill
1. *Reached stationarity, that is converged to the target distribution*
\vfill
2. *Is efficiently mixing, or in other words, the particle can sweep through the distribution without getting stuck*
\newpage

\subsection{Model Checking with the Posterior Predictive Distribution}

The posterior predictive distribution, _$p(y^{*}|y)$ is a powerful tool for model checking._
\vfill
...
*Our model permits negative values and values greater than 100. The data also suggests that additional mass should be placed at 0.*
